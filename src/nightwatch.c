...
int input_available(int n) // Checks if any letter(q for exiting) is available
                           // when nightwatch executes
{
  struct timeval val;
  val.tv_usec = 0;
  val.tv_sec = n;
  ...
  // Basic idea:
  // select function watches the given file decriptor sets (input,output, and
  // error) Checks if any of them is ready Waits for val time => Time feature is
  // exploited here rather than sleep(5) Advantage => Gives immediate result (if
  // sleep(5), checkes once in 5 sec)
